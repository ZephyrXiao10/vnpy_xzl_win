...
class EventEngine:
    ...
    def __init__(self, interval: int = 1):
        ...
        self._thread: Thread = Thread(target=self._run)         # 线程一：把event拿去process，处理response之类？ 
        self._timer: Thread = Thread(target=self._run_timer)    # 线程二：timer event，固定间隔接收数据之类？put event？
